    ...
    .map((category) => {
      ...
      const matchingCategory = categoriesData.find((cat: any) => {
        ...
        return (
          ...
          categoryLower.includes(catName.replace(/s$/, ''))
...
export default async function FarmListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getStateInfo = () => {
    ...
    const state = statesData.find(
      (s) => s.state_name === farm.state_province || s.state_code === farm.state_province
    );
      ...
      <main className="container mx-auto px-4 lg:px-8 py-12 max-w-7xl">
        ...
        <div className="bg-primary border border-primary/50 p-4 rounded-xl mb-6">
          <p className="text-white font-medium">
            <strong>Opening Date:</strong> {farm.opening_date} • <strong>Closing Date:</strong>{' '}
            {farm.closing_date}
          </p>
        </div>
          ...
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl mb-6 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-800 text-sm leading-relaxed">
                This farm's information has not been verified by the owner. We recommend calling
                ahead to confirm hours, availability, and pricing before visiting.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="lg:col-span-2 space-y-4">
            <Card className="shadow-sm border-0 bg-white">
              <CardHeader className="pb-6">
                ...
                {/* Farm Type Badge */}
                <div>
                  <Badge className="text-sm font-medium">{farm.type}</Badge>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-4">
                {Object.keys(farm).filter((key) => key.endsWith('_hours')).length > 0 ? (
                        ...
                        >
                          ...
                          <span
                            className={`font-medium ${
                              farm[key as keyof typeof farm] === 'Closed'
                                ? 'text-red-500'
                                : 'text-emerald-600'
                            }`}
                          >
                  ...
                  <p className="text-muted-foreground">
                    Operating hours not available. Please contact the farm for more information.
                  </p>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">
                    {farm.price_range || 'Contact for pricing'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-foreground">
                      {farm.opening_date ||
                        (farm.categories?.toLowerCase().includes('christmas')
                          ? 'Late November'
                          : 'Coming soon')}
                    </p>
                  </div>
                ...
                <div>
                  ...
                  {farm.payment_methods ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      Contact farm for payment options
                    </p>
                  )}
                </div>
