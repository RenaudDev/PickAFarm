...
export default function SearchResultsContent({ params }: SearchResultsContentProps) {
  ...
  // Filter farms by category (must be before conditional return for hooks)
  ...
  const cityLocation = useMemo((): UserLocation | null => {
    if (!locationData || !validFarms || validFarms.length === 0) return null;
  ...
  }, [categoryData, validFarms.length, locationData]);
  ...
  // If no location found, show error state (after all hooks)
  if (!locationData) {
    return (
