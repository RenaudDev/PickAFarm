...
export async function generateMetadata({
  ...
  if (stateData) {
    ...
    const topCategories = stateData.categories
      .slice(0, 3)
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {stateData.cities.map((city: any, index: number) => (
                  <Link key={index} href={`/farms-near/${city.slug}`}>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center text-sm text-muted-foreground">
                              ...
                              <span>
                                {city.farm_count} {city.farm_count === 1 ? 'farm' : 'farms'}
                              </span>
                            </div>
                          </div>
                        </div>
        ...
        {stateData.categories.length > 0 && (
          <section className="py-16 px-4 bg-background">
            <div className="max-w-6xl mx-auto">
              ...
              <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">
                Popular u-pick experiences available across the{' '}
                {stateData.geographic_type.toLowerCase()}
              </p>
        ...
        <section className="py-16 px-4 bg-muted/30">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg text-muted-foreground mb-6">
              {stateData.state_name} is home to {stateData.total_farms} pick-your-own farms offering
              fresh, locally-grown produce and memorable agritourism experiences. From{' '}
              {stateData.categories[0]?.name.toLowerCase() || 'seasonal farms'} to{' '}
              {stateData.categories[1]?.name.toLowerCase() || 'family farms'}, discover the best
              u-pick destinations across the {stateData.geographic_type.toLowerCase()}.
            </p>
            <p className="text-lg text-muted-foreground">
              Use the interactive map above to find farms near you, check ratings and reviews, get
              directions, and plan your visit. Save your favorite farms to your account to receive
              updates on seasonal availability and special events.
            </p>
          </div>
        </section>
      </main>
...
async function CategoryPage({ category, slug }: { category: any; slug: string }) {
  ...
  const getFeaturedFarms = () => {
      ...
      loc.farms?.forEach((farm: any) => {
        ...
        if (!allMatchingFarms.find((f) => f.id === farm.id)) {
          allMatchingFarms.push({
            ...
            url: `/farms/${farm.slug}`,
            blurb:
              farm.description || `Experience ${category.name.toLowerCase()} at this local farm.`,
    ...
    return allMatchingFarms.filter((farm) => farm.featured === true).slice(0, 3);
  };
  ...
  const usStates = statesWithCategoryFarms.filter(
    (state: any) => state.country === 'United States'
  );
  const canadianProvinces = statesWithCategoryFarms.filter(
    (state: any) => state.country === 'Canada'
  );
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center text-sm text-muted-foreground">
                            <MapPin className="h-4 w-4 mr-1" />
                            <span>
                              {state.categoryFarmCount}{' '}
                              {state.categoryFarmCount === 1 ? 'farm' : 'farms'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center text-sm text-muted-foreground">
                            <MapPin className="h-4 w-4 mr-1" />
                            <span>
                              {province.categoryFarmCount}{' '}
                              {province.categoryFarmCount === 1 ? 'farm' : 'farms'}
                            </span>
                          </div>
                        </div>
                      </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12">
            Popular Locations for {enrichedCategory.name}
          </h2>
          {enrichedCategory.topCities.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {enrichedCategory.topCities.map((city: any) => (
                <Link key={city.slug} href={`/${slug}/near/${city.slug}`}>
                  <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer border-l-4 border-l-primary group">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div>
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">
                No locations found with {enrichedCategory.name.toLowerCase()} farms.
              </p>
            </div>
          )}
        </div>
        ...
        <section className="py-16 px-4 bg-muted/30">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <p className="text-muted-foreground">
                Discover some of our top-rated {enrichedCategory.name.toLowerCase()} from our
                collection of {enrichedCategory.totalFarms}{' '}
            ...
            <div className="grid md:grid-cols-2 gap-8 items-start">
              <div>
                ...
                <p className="text-muted-foreground leading-relaxed mb-6">
                  Christmas tree farms offer a unique blend of agricultural tradition and holiday
                  magic. Unlike store-bought trees, farm-fresh trees are cut at peak freshness,
                  ensuring longer-lasting needles and superior fragrance that fills your home with
                  the authentic scent of the season.
                </p>
              </div>
              <div>
                <h3 className="text-xl font-semibold mb-4 flex items-center">
                ...
                <p className="text-muted-foreground leading-relaxed mb-6">
                  Most farms operate from late November through December, offering the perfect
                  family outing during the holiday season. Many provide additional activities like
                  hayrides, hot cocoa, and holiday decorations, creating memorable experiences that
