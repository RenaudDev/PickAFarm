...
export default async function FarmsNearCities({ params }: { params: Promise<{ cities: string }> }) {
  ...
  const locationData = locationsWithFarms.find(
    (loc) => loc.location_slug === resolvedParams.cities
  );
  ...
  const stateData = statesWithFarms.find(
    (s) => s.state_name.toLowerCase() === locationData.province.toLowerCase()
  );
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(generateCityPageSchema(sortedFarms, locationData)),
        }}
      />
