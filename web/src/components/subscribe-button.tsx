    ...
    async function checkIfSubscribed() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `🔔 Bell animation: ${!alreadySubscribed ? 'ENABLED' : 'DISABLED'} (subscribed: ${alreadySubscribed})`
          );
        }
      } catch (error) {
  ...
  return (
    <>
      <Button
        ...
        disabled={isLoading}
        aria-label={
          isSubscribed ? `Unsubscribe from ${farmName}` : `Subscribe to ${farmName} updates`
        }
      >
        <Bell
          className={cn(
        ...
        />
        {variant !== 'icon' && (isLoading ? '...' : isSubscribed ? 'Subscribed' : 'Subscribe')}
      </Button>
