  ...
  return {
    ...
    itemListElement: farms.map((farm, index) => ({
      ...
      item: {
        ...
        description:
          farm.description || `Visit ${farm.name} for pick-your-own produce and farm activities`,
        ...(farm.image && {
          image: farm.image,
        }),
        ...(farm.city_name &&
 ...
 */
export function generateMapItemListSchema(farms: Farm[], pageUrl: string, listName: string) {
  return {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: listName,
