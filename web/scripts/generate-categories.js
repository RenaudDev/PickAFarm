...
async function generateCategories() {
  ...
  try {
    ...
    if (!fs.existsSync(categoryContentPath)) {
      throw new Error(
        'category-content.json not found. This file defines which categories to create.'
      );
    }
    ...
    farms.forEach((farm) => {
      ...
      farmCategories.forEach((farmCategory) => {
        ...
        const matchingCuratedKey = Object.keys(curatedContent).find((curatedKey) => {
          ...
          const commonWords = curatedWords.filter(
            (word) => farmWords.some((farmWord) => farmWord === word) // Exact word match only
          );
        ...
        if (matchingCuratedKey) {
          ...
          if (!farmsByCategory[matchingCuratedKey].find((f) => f.id === farm.id)) {
            farmsByCategory[matchingCuratedKey].push({
              ...
              slug: farm.slug,
              url: `/farms/${farm.slug}`,
              blurb: farm.description
                ? farm.description.substring(0, 120) + '...'
                : 'Farm description coming soon...',
              city: farm.city_name,
    ...
    console.log(
      `📊 Generated ${finalCategories.length} categories (${finalCategories.filter((c) => c.totalFarms > 0).length} with farms)`
    );
    ...
    finalCategories.forEach((cat) => {
      const status =
        cat.totalFarms > 0 ? `${cat.totalFarms} farms` : 'no farms (page not generated)';
      console.log(`   ${cat.slug}: ${status}`);
    });

    console.log('\n🎉 Category generation complete!');
    console.log(
      `📊 Only manually curated categories are included. Add new categories to category-content.json to create new pages.`
    );
  } catch (error) {
