...
async function generateSearchData() {
  ...
  try {
    ...
    const farmsResponse = await fetch(
      'https://pickafarm-api.94623956quebecinc.workers.dev/api/farms'
    );

    if (!farmsResponse.ok) {
      throw new Error(
        `Farms API request failed: ${farmsResponse.status} ${farmsResponse.statusText}`
      );
    }
    ...
    const citiesResponse = await fetch(
      'https://pickafarm-api.94623956quebecinc.workers.dev/api/cities'
    );

    if (!citiesResponse.ok) {
      throw new Error(
        `Cities API request failed: ${citiesResponse.status} ${citiesResponse.statusText}`
      );
    }
...
`;
    ...
    console.log(
      `Categories: ${farmCategories.slice(0, 5).join(', ')}${farmCategories.length > 5 ? '...' : ''}`
    );
    console.log(
      `Locations: ${locations.slice(0, 5).join(', ')}${locations.length > 5 ? '...' : ''}`
    );
  } catch (error) {
    ...
    const fallbackData = {
      categories: ['Apple Picking', 'Pumpkin Patches', 'Berry Picking', 'Christmas Trees'],
      locations: [
        'Toronto, Ontario',
        'Rochester, New York',
        'Ottawa, Ontario',
