  ...
  try {
    ...
    if (response.status === 'OK') {
      ...
      for (const place of response.results) {
        ...
        if (!types.includes('locality') || types.includes('sublocality') || !place.name || place.name.length <= 2) {
          continue;
        }
        
        const distance_km = haversineDistance(latitude, longitude, place.geometry.location.lat, place.geometry.location.lng);
    ...
    console.error(`Error finding population centers near ${latitude}, ${longitude}:`, error.message);
    return [];
  }
}
...
async function fetchFarmsFromD1() {
  ...
  try {
    ...
    if (data.success && data.result) {
      ...
      return farms
        .filter(row => row.latitude && row.longitude && parseFloat(row.latitude) !== 0 && parseFloat(row.longitude) !== 0)
        .map(row => ({
          id: row.id,
          name: row.name,
...
(async function main() {
  ...
  if (!farms) {
    ...
    if (!fs.existsSync(farmsPath)) {
      console.error('❌ No farms data available. Please check D1 connection or ensure farms.json exists.');
      process.exit(1);
    }
    
    const farmsAll = loadJson(farmsPath);
    farms = farmsAll.filter(f => (f.active === 1 || f.active === true) &&
  ...
  const clusterList = Array.from(clusters.values())
    ...
    .filter(c => c.country === 'Canada' || c.country === 'United States' || c.country === 'USA' || c.country === 'US') // US and Canada
...
function getProvinceAbbrev(province) {
  ...
  const duplicateSlugs = Array.from(slugCounts.entries()).filter(([, n]) => n > 1).map(([slug, n]) => ({ slug, count: n }));
